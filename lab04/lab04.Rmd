...
Fill in the blanks in the code below to make a histogram of lexical decision times across all words, using a binwidth of 20 milliseconds.
...
```{r ldt_aoa_scatter, exercise = TRUE, exercise.eval=FALSE}
___ %>%
  ggplot(aes(x = ___, y = ___)) +
  ___()
```

```{r ldt_aoa_scatter-solution}
elp %>%
  ggplot(aes(x = Age_Of_Acquisition, y = LDT)) +
  geom_point()
```

```{r ldt_aoa_scatter-check}
grade_this_code()
```
